...
/// ```
///
/// **Size.** Defaults to the style's intrinsic size ([HeroIconStyle.nominalSize] — 24 for outline/solid, 20 for mini) so the icon matches the Finesse design pixel-for-pixel. Pass [size] to override; the icon always stays square.
///
/// **Color.** Heroicons paint with `currentColor`, so the whole glyph is tinted uniformly. The color resolves to [color] when given, otherwise the ambient [IconTheme] color, falling back to the theme's `onSurface`. This means a `GHHeroIcon` placed inside a button, list tile or `IconTheme` inherits the surrounding content color just like a Material [Icon].
class GHHeroIcon extends StatelessWidget {
  /// Creates a Heroicon for [icon] in the given [style].
  const GHHeroIcon(this.icon, {this.style = HeroIconStyle.outline, this.size, this.color, this.semanticLabel, super.key});
  ...
  /// An accessibility label announced for the icon. When `null`, the icon is treated as decorative and excluded from the semantics tree.
